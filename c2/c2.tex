...
\begin{definition}[Support]
    The \textbf{support} of an $R$-module $M$ is the set of prime ideals s.t. $M_p := S^{-1}M$ where $S = R \smallsetminus p$.
\end{definition}

\begin{remark}
    If $M$ is finitely generated, then an equivalent definition for support is
    \[
        \mathrm{Supp} (M) = \{ p \text{ prime ideal } \mid\ p\supseteq \Ann_R(M) \}
    \]
\end{remark}

\begin{proof}[Proof of Proposition \ref{prop:Noeth finite minimal prime}]
    If $M$ is a finitely generated $R$-module and $N\subseteq M$ a submodule, notice $\mathrm{Supp}(M) = \mathrm{Supp}(N) \cup \mathrm{Supp}(M/N)$. as $\Ann_R(N) \subseteq \Ann_R(M)$ and so is the case for the quotient. Then
    \[
        \mathrm{Supp}(M) = \bigcup_{i=1}^n \mathrm{Supp}(M_i/M_{i-1}) = \bigcup_{i=1}^n \mathrm{Supp}(R/p_i) = \{ \text{prime ideals of }R \text{ containing } p_i \}
    \]
